...
CONFIG = {
 ...
 CR + "/combat.md": {"mode": "sections", "ns": "combat", "cat": "core", "type": "rule", "tier": "core", "base_tags": ["combat"],
  "why": "User-ruled breakdown: rule sections plus one block per named catalog action (some actions are already snippets).",
  "sections": [
    ("combat-basics", "combat-basics", {}),
    ("attacking-and-defending", "attacking-and-defending", {"end": "cover", "note": "Merged: The Attack Roll + Advantage and Disadvantage in Combat (per ruling)."}),
    ("cover", "cover", {}),
    ("surprise-attacks", "surprise-attacks", {}),
    ("steps-to-an-attack", "steps-to-an-attack", {}),
    ("damage", "damage", {"end": "damage-sources", "note": "Merged: formula + damage dice (via snippet include) + modifiers + types (per ruling)."}),
    ("damage-sources", "damage-sources", {}),
    ("damage-resistance-and-weakness", "damage-resistance", {}),
    ("actions-in-combat", "actions-in-combat", {"end": "aggressive-actions", "note": "Aggressive/defensive classification intro; its own block so action blocks can reference it."}),
  ],
  "bold_blocks": [
    ("aggressive-actions", "defensive-actions", [
      ("**Standard Attack**", "standard-attack", ["combat", "action", "aggressive"]),
      ("**Disarm**", "disarm", ["combat", "action", "aggressive"]),
      ("**Draw a Weapon**", "draw-a-weapon", ["combat", "action", "aggressive"]),
      ("**Shatter**", "shatter", ["combat", "action", "aggressive"]),
      ("**Stand**", "stand", ["combat", "action", "aggressive"]),
      ("**Trip**", "trip", ["combat", "action", "aggressive"]),
      ("**Unarmed Attack**", "unarmed-attack", ["combat", "action", "aggressive"]),
    ]),
    ("defensive-actions", "related", [
      ("**Brace**", "brace", ["combat", "action", "defensive"]),
      ("**Use Cover**", "use-cover", ["combat", "action", "defensive"]),
    ]),
  ]},
 ...
 CR + "/environmental-effects.md": {"mode": "sections", "ns": "environment", "cat": "core", "type": "rule", "tier": "core", "base_tags": ["environment"],
  ...
  "sections": [
    ("environmental-effects", "environmental-effects", {"end": "extreme-environments", "note": "Single block per ruling: exposure intervals, heat & cold, toxic atmosphere, visibility."}),
  ]},
 ...
 SF + "/sci-fi-weapons.md": {"mode": "details", "cat": "sci-fi", "tier": "module",
  ...
  "det_map": {
    "light-melee-weapons": ("sci-fi-equipment", "equipment", ["equipment", "sci-fi", "weapon", "light-melee", "melee-weapon"]),
    "medium-melee-weapons": ("sci-fi-equipment", "equipment", ["equipment", "sci-fi", "weapon", "medium-melee", "melee-weapon"]),
    "pistols": ("sci-fi-equipment", "equipment", ["equipment", "sci-fi", "weapon", "pistol", "ranged-weapon"]),
    "rifles": ("sci-fi-equipment", "equipment", ["equipment", "sci-fi", "weapon", "rifle", "ranged-weapon"]),
    "heavy-ranged-weapons": ("sci-fi-equipment", "equipment", ["equipment", "sci-fi", "weapon", "heavy-ranged", "ranged-weapon"]),
...
SPLIT_DETAILS = {
 (IE + "/generic-equipment.md", "Pouch Sets"): [
   ("pouch-set-ammo", "pouch-set-ammo"), ("pouch-set-small", "pouch-set-small"), ("pouch-set-standard", "pouch-set-standard")],
 (IE + "/generic-equipment.md", "Bandages"): [
   ("bandages-large", "bandages-large"), ("bandages-small", "bandages-small")],
 (SF + "/bots--drones/components.md", "Signal Masts"): [
   ("signal-mast-retractable", "signal-mast-retractable"), ("signal-mast-heavy", "signal-mast-heavy")],
}
...
for page, cfg in CONFIG.items():
    ...
    for (anchor, ident, opts) in cfg.get("sections", []):
        ...
        add_block(b)
    # bold-entry blocks (combat action catalog)
    for (a_start, a_end, entries) in cfg.get("bold_blocks", []):
        hs = heading_by_anchor(page, a_start)
        he = heading_by_anchor(page, a_end)
        with open(os.path.join(REPO, page)) as f:
            plines = f.read().split("\n")
        lo, hi = hs["line"], (he["line"] - 1 if he else P["n_lines"])
        for (prefix, slug, tags) in entries:
            found = [i for i in range(lo, hi + 1) if plines[i - 1].startswith(prefix)]
            assert len(found) == 1, (page, prefix, found)
            start = found[0]
            end = start
            while end + 1 <= hi and plines[end].strip() != "" and not plines[end].startswith("**") and "include" not in plines[end]:
                end += 1
            b = {"id": f"combat/{slug}", "title": prefix.strip("*"), "page": page, "anchor": None,
                 "span": [start, end], "home": "snippet (proposed)",
                 "file": f"content/snippets/combat/{slug}.md",
                 "category": cfg.get("cat"), "type": "rule", "tier": cfg.get("tier"),
                 "tags": tags, "flags": [], "notes": "Named combat action (per ruling: individual blocks)."}
            add_block(b)
    # details
    if mode == "details":
        ...
        for d in P["details"]:
            ...
            if (page, d["title"]) in SPLIT_DETAILS:
                items = SPLIT_DETAILS[(page, d["title"])]
                hs = [h for h in inner if h["level"] >= 3]
                assert len(hs) == len(items), (page, d["title"], [h["anchor"] for h in hs])
                for idx, ((slug, anch), h) in enumerate(zip(items, hs)):
                    end = hs[idx + 1]["line"] - 1 if idx + 1 < len(hs) else d["end"] - 1
                    b = {"id": f"{ns}/{slug}", "title": h["text"], "page": page, "anchor": h["anchor"],
                         "span": [h["line"], end], "home": "snippet (proposed)",
                         "file": f"content/snippets/{ns}/{slug}.md",
                         "category": "sci-fi" if "sci-fi" in tags else cfg.get("cat"),
                         "type": btype, "tier": "module" if "sci-fi" in tags else cfg.get("tier"),
                         "tags": list(tags), "flags": ["split-from: " + d["title"]], "notes": ""}
                    add_block(b)
                continue
...
for path in RAW:
    if not path.startswith("content/snippets/") or path.endswith("_index.md"):
        continue
    rel_id = path[len("content/snippets/"):-3]
    BY_ID = {v[0]: v for v in SNIPPETS.values()}
    if rel_id not in BY_ID:
        print("!! unmapped snippet", rel_id); continue
    bid, cat, btype, tier, tags, excl = BY_ID[rel_id]
    b = {"id": bid, "title": rel_id.split("/")[-1].replace("-", " ").title(), "page": path, "anchor": None,
...
for b in blocks:
    ...
    b["selectable"] = b["id"] not in NOT_SELECTABLE
    if b["id"] in COMPUTERS and "computers" not in b["tags"]:
        b["tags"].append("computers")
...
# effective per-page anchor sets: own headings + headings of included snippets (recursive)
SNIP_BY_TARGET = {}
for path in RAW:
    if path.startswith("content/snippets/"):
        SNIP_BY_TARGET["/" + path[len("content/"):-3]] = path

def snippet_headings(path, seen=None):
    seen = seen or set()
    if path in seen:
        return []
    seen.add(path)
    out = [h["anchor"] for h in RAW[path]["headings"]]
    for inc in RAW[path]["includes"]:
        t = inc["target"]
        t = t if t.startswith("/") else "/" + t
        t = t[:-3] if t.endswith(".md") else t
        sp = SNIP_BY_TARGET.get(t)
        if sp:
            out += snippet_headings(sp, seen)
    return out

EFFECTIVE_ANCHORS = {}
for path in RAW:
    anchors = set(h["anchor"] for h in RAW[path]["headings"])
    for inc in RAW[path]["includes"]:
        t = inc["target"]
        t = t if t.startswith("/") else "/" + t
        t = t[:-3] if t.endswith(".md") else t
        sp = SNIP_BY_TARGET.get(t)
        if sp:
            anchors.update(snippet_headings(sp))
    EFFECTIVE_ANCHORS[path] = anchors
...
for page, P in RAW.items():
    ...
    for lk in P["links"]:
        ...
        if tpage is None:
            ...
            e["type"] = "reference"
            e["queued"] = False; e["class"] = "known-broken"
        ...
        if frag:
            ...
            if bid is None:
                if frag in EFFECTIVE_ANCHORS[tpage]:
                    e["target"] = f"page:{url_for(tpage)}#{frag}"
                else:
                    e["target"] = f"unresolved:{url}"
                    e["queued"] = False; e["class"] = "broken-anchor"
                    edges.append(e); unresolved.append(e); continue
            else:
                e["target"] = bid
        else:
        ...
        if pair in (("traits/particularly-attractive", "traits/unremarkable"), ("traits/unremarkable", "traits/particularly-attractive")):
            ...
            e["class"] = SUSPECT_PAIRS[pair]  # ruled: reference
            ...
            e["type"] = "dependency"; e["class"] = "prerequisite"  # ruled: auto-include
        elif page in EQUIP_PAGES and e["target"].startswith("item-tags/"):
            e["class"] = "tag-definition"  # ruled: reference; preflight aggregates
        edges.append(e)
    # includes
...
for (s, t, cls, why) in IMPLICIT:
    edges.append({"source": s, "target": t, "type": "reference", "queued": False,
                  "class": cls, "url": None, "text": None,
                  "file": None, "line": None, "note": why + " (implicit — no link in text; ruled reference)"})

# requires: feature-prereq dependencies onto source blocks
for e in edges:
    if e.get("type") == "dependency" and e["source"] in blocks_by_id:
        blocks_by_id[e["source"]].setdefault("requires", [])
        if e["target"] not in blocks_by_id[e["source"]]["requires"]:
            blocks_by_id[e["source"]]["requires"].append(e["target"])
...
# ---------- write outputs ----------
for b in blocks:
    if not isinstance(b["category"], list):
