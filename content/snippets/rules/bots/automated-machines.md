...
**Robot Chassis** — A machine's chassis lacks biological healing, but otherwise uses the same DEF/VIT health system that creatures and characters use.
...
**Power Source** — Automated machines have a [Battery](/docs/free-srd/inventory--equipment/item-tags/#battery) built into their chassis. The size of the battery is determined by the size of the machine (medium machines have medium batteries, small machines have small batteries, etc.). These batteries power all functions for the automated machine, and often do not need to be replaced under normal circumstances.

Automated machines function much like people do in that they require rest. During a rest they enter Rest Mode for six hours to resolve caching conflicts, free up memory, and prevent battery degradation. Every 16 hours an automated machine goes without a successful Rest Mode session requires 2 supply rolls of their primary power source. When their battery is depleted, they receive the [Dead Battery](/docs/free-srd/core-rules/sci-fi-additions/#dead-battery) condition. As long as an automated machine goes through regular Rest Mode cycles, its battery will not be an issue.

It takes ten minutes and use of the tech kit with supplies to successfully replace an automated machine's battery.

**Upgradeable** — Automated machines can be upgraded with [components](/docs/free-srd/inventory--equipment/sci-fi-equipment/bot-components/), however, the number of components is limited by the battery size of the automated machine.
- Micro and Small Batteries — 1 component
